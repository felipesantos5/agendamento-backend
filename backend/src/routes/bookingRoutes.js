...
router.get("/:barberId/monthly-availability", async (req, res) => {
  try {
    ...
    // Mapeia os dias de trabalho do barbeiro para facilitar a busca
    ...
    for (const day of daysInMonth) {
      ...
      // Causa de indisponibilidade #1: Dia bloqueado
      ...
      // Causa de indisponibilidade #2: Feriado
      ...
      // Causa de indisponibilidade #3: Barbeiro não trabalha no dia da semana
      ...
      // Causa de indisponibilidade #4: Nenhum horário vago no dia
      // (Lógica otimizada da rota `free-slots`)
      let hasAvailableSlot = false;
      const slotInterval = 15; // Intervalo para verificar os horários
      ...
      while (currentSlotTime < dayEnd) {
        ...
        // Se não encontrou nenhum conflito, achamos um horário livre!
        if (!hasConflict) {
          hasAvailableSlot = true;
          break; // Para a verificação deste dia e vai para o próximo
        }
