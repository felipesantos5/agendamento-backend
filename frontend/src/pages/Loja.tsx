...
export const Loja = () => {
  ...
  return (
    <>
      <div className="max-w-lg mx-auto mt-8 p-6 rounded-lg border bg-white shadow">
        {/* <h1 className="text-3xl font-bold mb-2">{barbershop.name}</h1>
        <div className="mb-5 text-gray-600">
          <div><b>Endereço:</b> {barbershop.address.rua}, {barbershop.address.numero} - {barbershop.address.bairro}, {barbershop.address.cidade} - {barbershop.address.estado}, {barbershop.address.cep}</div>
          <div><b>Contato:</b> {barbershop.contact}</div>
          <div className="mt-2"><b>Horário de funcionamento:</b></div>
          <ul>
            {barbershop.workingHours.map(w =>
              <li key={w.day}>{w.day}: {w.start} - {w.end}</li>
            )}
          </ul>
        </div> */}
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <div>
            ...
            <Input value={customerWhatsapp} onChange={e => setCustomerWhatsapp(e.target.value)} placeholder="(99) 99999-9999" />
          </div>
          <div>
            ...
            <Select value={selectedService} onValueChange={setSelectedService}>
              ...
              <SelectContent>
                {services.map(s =>
                  <SelectItem key={s._id} value={s._id}>{s.name} - R$ {s.price}</SelectItem>
                )}
              </SelectContent>
            </Select>
          </div>
          ...
          {/* Horário disponível só de exemplo, no real o backend dirá se está livre. */}
